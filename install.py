...
def download_deps( mcp_dir, download_mc, forgedep=False ):
    ...
    if not forgedep:
        flat_lib_dir = getLibDirVanilla(base_dir)
        flat_native_dir = os.path.join(flat_lib_dir,"natives",native)
    else:
        flat_lib_dir = getLibDirForge(base_dir)
        flat_native_dir = os.path.join(flat_lib_dir,"natives",native)
...
def setupForgeSrc(mcp_dir, mcp_dir_clean, clean, isForge):
    ...
    lib_dir = os.path.join(getLibDir(base_dir), full_forge_version)
    ...
    if isForge:
        doForgeFileDiffs(mcp_dir=mcp_dir,
                         mcp_dir_clean=mcp_dir_clean,
                         minecrift_forge_src_dir=os.path.join(base_dir, 'forgesrc'),
                         mcp_patch_dir=os.path.join(base_dir, 'mcppatches'))
